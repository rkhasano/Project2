...
{
	//Count turns if there will be no winner
	int numberOfTurns=9;
	bool noWinner = true;
	
	while (numberOfTurns>0 && noWinner)
	{
		int Xcoor, Ycoor;
		DrawStats();
		DrawBoard();
		...
		Xcoor = GetValidInput(0, 2);
		
		//Ask user to enter Y coordinate.
		cout << "Player " << turn << ", enter an Y coordiante: ";
		//Check if it is in range.
		Ycoor = GetValidInput(0, 2);
		
		if (PlaceMarker(Xcoor, Ycoor, playerMarker[turn])) 
		{
			SwapTurn();
			numberOfTurns--;
		}
...
{
	//Character arrays to store values from board.
	char raw[3], column[3];
	char diagonal1[3], diagonal2[3];
     for(int i=0;i<3;i++)
	 {
		 diagonal1[i] = gameBoard[i][i];
		 diagonal2[i] = gameBoard[2-i][i];
	  ...
	  if ((raw[0] == raw[1] && raw[0] == raw[2]) && raw[0] != ' ')
	  {
		  if (raw[0] == 'x') return 0;
		  else return 1;
	  }
	  ...
	  {
		  if (column[0] == 'x') return 0;
		  else return 1;
	  }
	  if ((diagonal1[0] == diagonal1[1] && diagonal1[0] == diagonal1[2]) && diagonal1[0] != ' ')
	  {
		  if (diagonal1[0] == 'x') return 0;
		  else return 1;
	  }
	  if ((diagonal2[0] == diagonal2[1] && diagonal2[0] == diagonal2[2]) && diagonal2[0] != ' ')
	  {
		  if (diagonal2[0] == 'x') return 0;
		  else return 1;
	  }
	 }
